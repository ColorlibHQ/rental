...
function rental_post_cats() {
    ...
    if( $cats ) {
        ...
        foreach( $cats as $cat ) {
           $categories .= '<a class="cat_item" href="' . esc_url( get_category_link( absint( $cat->term_id ) ) ) . '">' .esc_html( $cat->name ) . '</a>';
        }
...
// social media
if ( ! function_exists( 'rental_social' ) ) {
	function rental_social( $args = array()  ){
		...
		$url = rental_opt('rental_social_url');
...
function rental_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'rental' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'rental' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'rental' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function rental_set_post_views($postID) {
    $count_key = 'rental_post_views_count';
...
// blog post categoty 
function rental_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'rental' ) );
